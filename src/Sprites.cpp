  ...
  switch (draw_mode) {
    case SPRITE_UNMASKED:
      ...
      for (uint8_t a = 0; a < loop_h; a++) {
        ...
        //   bitmap_data = pgm_read_byte(bofs) * mul_amt;           //TODO 可能有问题
    ...
    case SPRITE_PLUS_MASK:
      ...
      uint8_t *Y = Arduboy2Base::sBuffer+ofs + 128;
          ...
          for (uint8_t iCol = 0; iCol < rendered_width; iCol++) 
          {
              ...
              if(yOffset != 0)
              {
                  ...
                  if(sRow < 7)
                  {
                      ...
                      temp = mask_data>>8;
                      temp = ~temp;
                      mask_data = (temp<<8) | (mask_data&0x00ff);
                      data &= mask_data>>8;
                      data |= bitmap_data>>8;
                      *Y++ = data;
                  }
              ...
              if(sRow >= 0)
              {
                  ...
                  temp = mask_data;
                  temp = ~temp;
                  mask_data = (temp) | (mask_data&0xff00);
                  data &= (uint8_t)mask_data;
                  data |= (uint8_t)bitmap_data;
                  *buffer_ofs++ = data;
              }
